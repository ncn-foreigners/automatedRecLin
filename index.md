...
from the `automatedRecLin` package (the Jaro-Winkler distance, i.e.,
...
Train a record linkage model using
...
Predict matches using the
[`predict()`](https://rdrr.io/r/stats/predict.html) method for
`rec_lin_model` objects. The output has a similar structure to that of
[`mec()`](https://ncn-foreigners.github.io/automatedRecLin/reference/mec.md).
...
to create comparison vectors that the model will be trained on.
...
a matrix as input for the
[`predict()`](https://rdrr.io/r/stats/predict.html) method for
`rec_lin_model` objects and that needs to be specified in the
`data_type` argument. Set `type = "response"` to ensure the XGBoost
model predicts the probability of matching (this argument may vary
depending on the model or library used).
...
Some elements of the package were designed with the support of
[StatsClaw](https://statsclaw.ai).
...
Workflow for Statistical Software
Development](https://arxiv.org/abs/2604.04871). arXiv preprint
arXiv:2604.04871.

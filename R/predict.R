...
#'
#' @param object A `rec_lin_model` object from [train_rec_lin()] or [custom_rec_lin_model()].
...
#' @param fixed_method A method for solving fixed-point equations using the \link[FixedPoint:FixedPoint]{FixedPoint()} function.
...
#'
#' @details
#' The [predict.rec_lin_model()] method estimates the probability/density ratio
#' between matches and non-matches for pairs in given
#' datasets, based on a model obtained using the
#' [train_rec_lin()] or [custom_rec_lin_model()].
...
#' (see [Lee et al. (2022)](https://www150.statcan.gc.ca/n1/pub/12-001-x/2022001/article/00007-eng.htm)).
#'
#' The [predict.rec_lin_model()] method allows the construction of the predicted set
...
predict.rec_lin_model <- function(object,
  ...
  if (!is.null(object$ml_model)) {
    ...
    Omega <- score_mec_ratio(Omega, object)
